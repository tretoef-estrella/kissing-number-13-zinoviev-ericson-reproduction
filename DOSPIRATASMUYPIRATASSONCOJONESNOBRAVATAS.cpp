// =====================================================================
// DOSPIRATASMUYPIRATASSONCOJONESNOBRAVATAS.cpp
// =====================================================================
// LUNA project, K(13) >= 1155 record attempt — Engine #2.
//
// Inherits DOSPIRATASDUROSCONTRESCOJONESPUROSENCERADOSYPERFUMADOS_v3
// architecture (5-chain hot-ladder PT, cost-probe, MIX_RECAL by
// productivity, triple-rescue with T modulation, D190 paso-1 in-engine
// verify) and adds the structural lessons from F-dospiratas-1 closure
// (paper LUNA_RESUMEN_COJONERIL_V7 wall #10, plateau gbest=2 HUB
// topology revealed F40-axial-recycling materialized in continuous SA).
//
// What is NEW vs v3 (engine #2 specific):
//   - Move P3 atomic 3-vector swap with HUB-targeting selection rule
//     (k1=arg-max viol_count, k2,k3=top conflict-partners of k1).
//   - F40 axial filter at pool generation (D36 axial-recycling check
//     hardened: filter ±4·e_i for i in [0..11] BUT NOT i=12 — coord-12
//     asymmetry per F55 finding documented in paper v14 v8).
//   - Pattern I = Q(zeta_12) cyclotomic candidates loaded from external
//     file pool_pattern_I.txt (Encerador delivers, Constructor consumes).
//     Interface is strict: the engine VALIDATES the file on load and
//     refuses to run if invariants fail.
//   - N_TARGET sweep mode: instead of fixed N=1155, the engine runs
//     successive targets {1146, 1154, 1155, 1158, 1162, 1170, 1188}
//     each with budget cap, auto-stop on plateau. The mathematical
//     justification is "natural-family closure size N*(A) is family-
//     dependent" (per Encerador audit addendum 06 May 2026), NOT
//     "non-monotonic K(n) jump" (theorem-violating framing rejected).
//   - D36 axial-recycling check at every gbest descent: alert if any
//     vector in current config is +/-4*e_i F40 axial.
//   - Separate code paths for ZE99-load and Pattern-F-extension (P1
//     hardening from main audit).
//   - Budget cap 24M iters per target (not 32M), auto-stop at 5M
//     plateau per target.
//   - D190 paso-4 verifier upgraded externally (d190_paso4_verify_v2.py)
//     adds provenance check + axial check + N>1155 monotonicity-subset
//     verification.
//
// What is UNCHANGED from v3 (architecture stays):
//   - 5-chain Parallel Tempering hot ladder T={0.05, 0.15, 0.40, 0.80, 1.40}.
//   - Cost-probe pre-SA per chain (Sobol decision matrix).
//   - MIX_RECAL by productivity (impr/tries), Move K/R fixed at w=10.
//   - Triple-rescue P1/P2/P3 with T modulation 1.20/1.45/1.80.
//   - Adaptive PROMOTE: revert to best_S + perturb 30 vertices.
//   - Bookkeeping incremental O(N), integrity check every 100k iters.
//   - Logging paper-grade per 500k iters.
//
// Cojoneril discipline applied (per Architect's instructions):
//   - Pattern I generation is NOT in this file. The engine LOADS the
//     pool from pool_pattern_I.txt produced by Encerador's Phase 3
//     sandbox with Cholesky+Fincke-Pohst rigor. The Constructor does
//     NOT invent Q(zeta_12) embedding logic. Cojones no bravatas.
//   - If pool_pattern_I.txt is missing or invalid, engine refuses to
//     run with PATTERN_I mode. Falls back to v3-pool mode for sanity
//     testing.
//   - Sanity test: engine in v3-pool mode (no Pattern I) MUST reproduce
//     DOSPIRATAS_v3 plateau gbest=2 with HUB topology (axial -4*e_11
//     hub + 2 ZE99 irrationals at IP=8sqrt(3)). Verified before any
//     Mac launch with Pattern I.
//
// Author: Rompehuevos Constructor Claude. Architect: Rafa, Madrid.
//         Mathematical Companion: Encerador de números cojoneros.
// Date: 06 May 2026 evening, Madrid.
// Build: g++ -O3 -march=native -std=c++17 -funroll-loops -Wall -Wextra
//        -o DOSPIRATAS_E2 DOSPIRATASMUYPIRATASSONCOJONESNOBRAVATAS.cpp
// Run:   cd ~/Downloads && caffeinate -dims taskpolicy -c utility ./DOSPIRATAS_E2
//          cojones_puros_final_1154.txt --mode=pattern-I --pool-i=pool_pattern_I.txt
//          --sweep=1146,1154,1155,1158,1162,1170,1188
//          2>&1 | tee DOSPIRATAS_E2_run.log
//
// Or for sanity test (no Pattern I, reproduce v3 plateau):
//   ./DOSPIRATAS_E2 cojones_puros_final_1154.txt --mode=v3-sanity
// =====================================================================
...
// Engine #2: N_TARGET is a RUNTIME variable to support sweep mode.
// The sweep iterates over a list of target sizes informed by the
// "natural-family closure size N*(A) family-dependent" framing
// (Encerador audit addendum 06 May 2026, NOT non-monotonic K(n) jump).
// Different algebraic families (ZE99 = Q+Q√3, Pattern I = Q(zeta_12))
// may have different intrinsic configuration sizes. The sweep targets
// values motivated by Q(zeta_12)'s 4-real-coord-per-cyclotomic-element
// structure plus historical anchors (1146 PackingStar, 1154 ZE99,
// 1155 record-by-1, then 1158/1162/1170/1188 = +4/+8/+16/+34).
//
// N_TARGET_MAX caps array sizing at compile time. Active N_TARGET is
// runtime via global g_N_target.
static constexpr int N_TARGET_MAX   = 1200;
static int           g_N_target     = 1155;   // overridden at runtime
#define N_TARGET (g_N_target)                  // legacy alias for inline use
...
// Default sweep values (Encerador's audit §5.1 recommendation, refinable
// by Phase 3 Q(zeta_12) cset profile). Values can be overridden via
// --sweep CLI flag.
static const std::vector<int> DEFAULT_SWEEP = {1146, 1154, 1155, 1158, 1162, 1170, 1188};

static constexpr int N_CHAINS       = 5;
// Hot ladder inherited from v3: chain 4 at T=1.40 crosses Δ=+5 barriers
// with prob 2.8%, Δ=+8 with 0.3%. Empirically verified in v3 to escape
// plateau-4 basin and reach plateau-2 sustained.
...
// Engine #2 budget per N_TARGET (Architect's instruction 06 May 2026):
//   - 24M iters cap per target (down from v3's 16M+8*2M=32M).
//   - Auto-stop at 5M iters of sustained plateau per target.
//   - With 7 targets in default sweep, total upper budget 168M iters.
//   - Mac M2 25% at ~7500 iters/s -> ~6.2h wall-clock if all targets
//     run full 24M; typical case with auto-stop ~3-4h total sweep.
static constexpr long long ITERS_PER_CHAIN_BASE = 16'000'000LL;
static constexpr long long EXTEND_BY            = 2'000'000LL;
static constexpr int       MAX_EXTENSIONS       = 4;     // 16M + 4*2M = 24M cap
static constexpr long long PLATEAU_AUTOSTOP     = 5'000'000LL;
...
// =====================================================================
// Engine #2 NEW: F40 axial detection (D36 axial-recycling check).
//
// Paper LUNA_PAPER_v14 §META v10 D36 + V7 F-dospiratas-1: vectors of
// the form ±4·e_i are F40 axial sacrifice points. Per cuarto F-prime Q3
// and BW_16 cross-cut findings (paper v14 §META v10), accepting these
// in pool extensions creates false-positive "novel" hits.
//
// COORD-12 ASYMMETRY (P0 audit fix): paper v14 §F55 documented coord-12
// asymmetry: while ±4·e_i for i ∈ [0..11] are F40 sacrifice axials,
// coord-12 is structurally distinct in ZE99's Q+Q√3 embedding (it's
// the irrational-paired coord). We do NOT filter ±4·e_12 because it's
// not in the F40 sacrifice family.
// =====================================================================
inline bool is_F40_axial(const Vec& v) {
    int n_nonzero = 0;
    int single_idx = -1;
    for (int i = 0; i < DIM; i++) {
        if (std::abs(v.c[i]) > EPS) {
            n_nonzero++;
            single_idx = i;
        }
    }
    if (n_nonzero != 1) return false;
    if (std::abs(std::abs(v.c[single_idx]) - 4.0) > EPS) return false;
    // P0 audit fix: coord-12 is asymmetric per F55, NOT in F40 family
    if (single_idx == 12) return false;
    return true;  // ±4·e_i for i ∈ [0..11]
}
...
static std::vector<Vec> extend_pool_cross_radical(
    ...
    int n_extra)
{
    // P1 audit fix (engine #2): separate code paths for ZE99-base vs
    // extension. Base is loaded verbatim by load_pool() and passed in
    // here; extensions are appended after with F40 axial filter applied
    // (per D36 / V7 §3.5).
    //
    // The F40 axial filter (is_F40_axial helper) rejects ±4·e_i for
    // i ∈ [0..11] BEFORE they enter the pool. This prevents the v3
    // plateau-2 HUB pattern from re-emerging through the same axial
    // generated by Pattern F twin-pair differences.
    //
    // Engine #2 axial-filter rejection counters logged at end.
    std::vector<Vec> out = base;
    out.reserve(base.size() + n_extra);

    int n_axial_rejected = 0;  // Engine #2 NEW: track F40 filter hits
    ...
    auto try_insert = [&](const Vec& v) -> bool {
        // Engine #2 NEW: F40 axial filter (D36 / V7 §3.5)
        if (is_F40_axial(v)) {
            n_axial_rejected++;
            return false;
        }
    ...
    // Budget split (engine #2 changes vs v3):
    //   - DROP Pattern A entirely (F-MASTER subset, paper v14 §47).
    //   - DROP Pattern E entirely (F-cinco-1 closure V4, LP=1154).
    //   - KEEP Pattern B at small share (Z[√3]-mixed complement).
    //   - KEEP Pattern F at largest share (best profile, axial-filtered).
    //   - Reserve slots for Pattern I = Q(ζ_12) cyclotomic loaded later.
    int target_A = 0;                                  // engine #2: DROP A
    int target_B = (n_extra * 20 + 99) / 100;          // 20%
    int target_E = 0;                                  // engine #2: DROP E
    int gen_A = 0, gen_B = 0, gen_E = 0;
    (void)target_A; (void)target_E; (void)gen_A; (void)gen_E;

    // Pattern A removed in engine #2.
    // Pattern E removed in engine #2.
    ...
    // share because its profile is best). target_A/B/E reduced accordingly.
    {
        ...
        fprintf(stdout, "[POOL_EXT engine#2] B=%d (target %d), F=%d (target %d), "
                "%zu pairs available | F40 axials rejected (D36 filter): %d\n",
                gen_B, target_B, gen_F, target_F,
                ip8_pairs.size(), n_axial_rejected);
        fprintf(stdout, "[POOL_EXT engine#2] Pattern A and Pattern E DROPPED "
                "(F-MASTER and F-cinco-1 closed respectively)\n");
        return out;
    }
}
...
// =====================================================================
// Engine #2 NEW: Pattern I = Q(ζ_12) cyclotomic loader.
//
// The Encerador delivers `pool_pattern_I.txt` with format:
//   Line 1: # Pattern I Q(zeta_12) candidates | size=N | norm2=16
//   Lines 2..N+1: 13 coords separated by comma in symbolic Cohn notation
//                 (e.g., "2*sqrt(3)", "-2*sqrt(3)", "1/2", etc.)
//                 OR float64 representation accurate to 1e-9.
//   Optional: trailing comment per line with template signature.
//
// Constructor's responsibility (validation, NOT generation):
//   1. Each vector parses to 13 float64 coordinates.
//   2. norm² = 16 ± 1e-9 universal.
//   3. NO ±4·e_i for i ∈ [0..11] (D36 filter — Encerador should pre-filter
//      but Constructor double-checks per discipline).
//   4. NO duplicates with ZE99 base (Encerador should pre-filter).
//   5. Header line present.
//
// Returns: vector<Vec> of validated Pattern I candidates (excluding header).
// Exits with [FATAL] if ANY validation fails.
// =====================================================================
static std::vector<Vec> load_pool_pattern_I(const char* path,
                                            const std::vector<Vec>& ze99_base)
{
    std::vector<Vec> out;
    std::ifstream fin(path);
    if (!fin) {
        fprintf(stderr, "[FATAL] cannot open Pattern I file: %s\n", path);
        fprintf(stderr, "[INFO]  Pattern I file produced by Encerador's Phase 3 sandbox.\n");
        fprintf(stderr, "[INFO]  Format: header + N lines of 13 comma-separated coords.\n");
        exit(1);
    }

    // Build ZE99 keyset for dedup check
    std::unordered_set<size_t> ze99_hashes;
    VecHash hasher;
    for (const auto& v : ze99_base) {
        ze99_hashes.insert(hasher(v));
    }

    std::string line;
    int linenum = 0;
    bool header_seen = false;
    int n_axial_in_file = 0;
    int n_norm_drift = 0;
    int n_ze99_dup = 0;
    int n_parsed = 0;
    ...
    while (std::getline(fin, line)) {
        ...
        if (line[firstnonspace] == '#') {
            if (!header_seen) {
                header_seen = true;
                fprintf(stdout, "[POOL_I] header: %s\n", line.c_str());
            }
            continue;
        }
        // Parse 13 comma-separated coordinates (float64 expected;
        // symbolic notation accepted via simple replacement).
        Vec v;
        std::stringstream ss(line);
        std::string token;
        int k = 0;
        const double S3_local = std::sqrt(3.0);
        while (std::getline(ss, token, ',') && k < DIM) {
            // Trim whitespace
            size_t a = token.find_first_not_of(" \t");
            size_t b = token.find_last_not_of(" \t");
            if (a == std::string::npos) { v.c[k++] = 0.0; continue; }
            std::string t = token.substr(a, b - a + 1);
            // Detect symbolic sqrt(3) notation
            if (t.find("sqrt") != std::string::npos) {
                // Patterns: "sqrt(3)", "-sqrt(3)", "2*sqrt(3)", "-2*sqrt(3)",
                //           "n*sqrt(3)"
                double coef = 1.0;
                if (t[0] == '-') {
                    coef = -1.0;
                ...
                if (star != std::string::npos) {
                    std::string num_part = t.substr(0, star);
                    coef *= std::stod(num_part);
                }
                v.c[k++] = coef * S3_local;
            } else {
                try {
                    ...
                    fprintf(stderr, "[FATAL] Pattern I line %d parse fail: '%s'\n",
                            linenum, t.c_str());
                    exit(1);
                }
            }
        ...
        if (k != DIM) {
            fprintf(stderr, "[FATAL] Pattern I line %d has %d coords, expected %d\n",
                    linenum, k, DIM);
            exit(1);
        }
        n_parsed++;

        // Validation 1: norm² = 16 ± 1e-9
        double n2 = v.norm2();
        if (std::abs(n2 - NORM2) > 1e-9) {
            n_norm_drift++;
            fprintf(stderr, "[POOL_I VALIDATION] line %d norm²=%.12f != 16 (drift)\n",
                    linenum, n2);
            continue;
        }

        // Validation 2: not F40 axial (D36 filter)
        if (is_F40_axial(v)) {
            n_axial_in_file++;
            fprintf(stderr, "[POOL_I VALIDATION] line %d is F40 axial ±4·e_i, REJECTED\n",
                    linenum);
            continue;
        }

        // Validation 3: not duplicate with ZE99
        size_t h = hasher(v);
        if (ze99_hashes.count(h)) {
            // Hash collision possible; do exact check
            bool dup = false;
            for (const auto& zv : ze99_base) {
                if (v == zv) { dup = true; break; }
            }
            if (dup) {
                n_ze99_dup++;
                continue;
    ...
    if (!header_seen) {
        fprintf(stderr, "[FATAL] Pattern I file missing header line "
                "'# Pattern I ...'\n");
        exit(1);
    }

    fprintf(stdout, "[POOL_I] loaded %zu Pattern I candidates (Q(ζ_12) cyclotomic)\n",
            out.size());
    fprintf(stdout, "[POOL_I] validation: parsed=%d, norm²-drift=%d, "
            "F40-axials=%d, ZE99-dups=%d\n",
            n_parsed, n_norm_drift, n_axial_in_file, n_ze99_dup);

    if (out.empty()) {
        fprintf(stderr, "[FATAL] Pattern I pool is empty after validation. "
                "Engine cannot run in pattern-I mode.\n");
        exit(1);
    }

    if (n_norm_drift > 0 || n_axial_in_file > 0) {
        fprintf(stderr, "[WARN] Pattern I file has %d norm-drift and %d "
                "axial-rejected. Encerador should pre-filter; flagging.\n",
                n_norm_drift, n_axial_in_file);
    }
...
struct State {
    ...
    // Engine #2 NEW: Move P3 atomic 3-vector swap.
    // Compute Δ for atomic triple replace: S[k1]<-nk1, S[k2]<-nk2, S[k3]<-nk3.
    // All k_i distinct; all nk_i distinct; all new indices not currently in S
    // (or only at the position they're being placed).
    // Selection rule expected (set externally): k1 = arg-max viol_count (HUB),
    // k2, k3 = top conflict-partners of k1.
    long long delta_triple_replace(int k1, int nk1, int k2, int nk2,
                                   int k3, int nk3) const {
        if (k1 == k2 || k1 == k3 || k2 == k3) return 0;
        if (nk1 == nk2 || nk1 == nk3 || nk2 == nk3) return 0;
        if (idx_in_S[nk1] != -1 && idx_in_S[nk1] != k1) return 0;
        if (idx_in_S[nk2] != -1 && idx_in_S[nk2] != k2) return 0;
        if (idx_in_S[nk3] != -1 && idx_in_S[nk3] != k3) return 0;
        const Vec& vo1 = (*pool)[S[k1]];
        const Vec& vo2 = (*pool)[S[k2]];
        const Vec& vo3 = (*pool)[S[k3]];
        const Vec& vn1 = (*pool)[nk1];
        const Vec& vn2 = (*pool)[nk2];
        const Vec& vn3 = (*pool)[nk3];
        long long old_v = 0;
        long long new_v = 0;
        // pairs involving k1, k2, or k3 with everyone else (excluding the
        // three internal pairs which we count separately)
        for (int i = 0; i < N_TARGET; i++) {
            ...
            if (ip(vi, vo2) > IP_THRESH + EPS) old_v++;
            if (ip(vi, vn2) > IP_THRESH + EPS) new_v++;
            if (ip(vi, vo3) > IP_THRESH + EPS) old_v++;
            if (ip(vi, vn3) > IP_THRESH + EPS) new_v++;
        }
        // 3 internal pairs: (k1,k2), (k1,k3), (k2,k3)
        if (ip(vo1, vo2) > IP_THRESH + EPS) old_v++;
        if (ip(vn1, vn2) > IP_THRESH + EPS) new_v++;
        if (ip(vo1, vo3) > IP_THRESH + EPS) old_v++;
        if (ip(vn1, vn3) > IP_THRESH + EPS) new_v++;
        if (ip(vo2, vo3) > IP_THRESH + EPS) old_v++;
        if (ip(vn2, vn3) > IP_THRESH + EPS) new_v++;
        return new_v - old_v;
    }

    void apply_triple_replace(int k1, int nk1, int k2, int nk2,
                              int k3, int nk3) {
        // Same approach as dual: sequential single replaces preserve
        // viol_count incremental tracking.
        apply_replace(k1, nk1);
        apply_replace(k2, nk2);
        apply_replace(k3, nk3);
    }
};
...
struct MoveMix {
    // Weights: S=single, P1=2-vector atomic, P2=2-vector targeted,
    //          P3=3-vector atomic HUB-targeting (engine #2 NEW),
    //          Q=rotational, K=kick, R=soft-restart.
    // Sum normally = 100 but recalibrated by productivity.
    int S_, P1_, P2_, P3_, Q_, K_, R_;
    int sum() const { return S_+P1_+P2_+P3_+Q_+K_+R_; }
    int pick(std::mt19937_64& rng) const {
        ...
        if (x < P2_) return 2;
        x -= P2_;
        if (x < P3_) return 6;     // Move P3 NEW (returns code 6)
        x -= P3_;
        if (x < Q_)  return 3;
        x -= Q_;
        if (x < K_)  return 4;
...
struct Chain {
    ...
    MoveMix mix;
    MoveStats st_S, st_P1, st_P2, st_P3, st_Q, st_K, st_R;
    int reheats_count = 0;
...
// =====================================================================
// Engine #2 NEW: forward declaration of Move P3 (defined below).
static bool sa_step_move_p3(Chain& ch, const std::vector<Vec>& pool,
                            std::mt19937_64& rng, double T_eff);
                    ...
                    double T_mod = 1.0)
{
    ...
    // Engine #2 NEW: Move P3 dispatch (code 6, atomic 3-vector swap).
    // sa_step_move_p3 forward-declared at global scope.
    if (move_type == 6) {
        return sa_step_move_p3(ch, pool, rng, T_eff);
    }
...
// =====================================================================
// Engine #2 NEW: Move P3 atomic 3-vector swap with HUB-targeting.
//
// Selection rule (informed by F-dospiratas-1 HUB topology):
//   1. k1 = HUB = arg-max viol_count[k]. The most-violating vertex.
//   2. k2, k3 = top 2 conflict-partners of k1, found by scanning the
//      pool-violation neighbors of k1 in current config S.
//   3. Replace S[k1], S[k2], S[k3] with random pool candidates (nk1,
//      nk2, nk3) chosen to minimize Δ_combined.
//   4. Single Metropolis on Δ_combined.
//
// This DIRECTLY ATTACKS the HUB topology seen in v3 plateau gbest=2:
//   v[1109], v[1131] = ZE99 irrationals, v[1154] = -4*e_11 axial hub.
//   Move P1 (2-vector) cannot resolve a 3-vertex coordination.
//   Move P3 can.
//
// Cost per attempt: ~3*N delta_combined evaluation = ~3500 ops in R^13.
// Expected productivity: 0.5-1.5% in HUB scenarios (higher than P1
// in coordinated-conflict regimes).
// =====================================================================
static bool sa_step_move_p3(Chain& ch, const std::vector<Vec>& pool,
                            std::mt19937_64& rng, double T_eff)
{
    ...
    // Step 1: find HUB k1 = arg-max viol_count.
    // Sample 32 random k indices and pick max (incremental hint cheap).
    std::uniform_int_distribution<int> k_dist(0, N_TARGET - 1);
    int k1 = -1;
    int best_viol = -1;
    for (int t = 0; t < 32; t++) {
    ...
    if (k1 < 0 || best_viol == 0) {
        // No conflict to attack; fall through to random k1.
        k1 = k_dist(rng);
    }

    // Step 2: find conflict-partners of k1.
    // A conflict-partner is k2 such that ip(S[k1], S[k2]) > IP_THRESH+EPS.
    // Scan S; collect up to MAX_PARTNERS partners.
    const int MAX_PARTNERS = 16;
    int partners[MAX_PARTNERS];
    int n_partners = 0;
    const Vec& v_k1 = pool[ch.state.S[k1]];
    for (int i = 0; i < N_TARGET && n_partners < MAX_PARTNERS; i++) {
        if (i == k1) continue;
        if (ip(v_k1, pool[ch.state.S[i]]) > IP_THRESH + EPS) {
            partners[n_partners++] = i;
        }
    }
    ...
    if (n_partners >= 2) {
        ...
        int p1_idx = std::uniform_int_distribution<int>(0, n_partners-1)(rng);
        int p2_idx = std::uniform_int_distribution<int>(0, n_partners-1)(rng);
        if (p1_idx == p2_idx) p2_idx = (p2_idx + 1) % n_partners;
        k2 = partners[p1_idx];
        k3 = partners[p2_idx];
    } else if (n_partners == 1) {
        // Only one partner. Pick partner + random.
        k2 = partners[0];
        k3 = k_dist(rng);
        if (k3 == k1 || k3 == k2) k3 = (k3 + 1) % N_TARGET;
    } else {
        // No conflict-partners. Pick two random.
        k2 = k_dist(rng);
        k3 = k_dist(rng);
        if (k2 == k1) k2 = (k2 + 1) % N_TARGET;
        if (k3 == k1 || k3 == k2) k3 = (k3 + 1) % N_TARGET;
    }
    if (k1 == k2 || k1 == k3 || k2 == k3) return false;

    // Step 3: pick random pool candidates nk1, nk2, nk3.
    std::uniform_int_distribution<int> p_dist(0, (int)P - 1);
    int nk1 = p_dist(rng);
    int nk2 = p_dist(rng);
    int nk3 = p_dist(rng);
    if (nk1 == nk2 || nk1 == nk3 || nk2 == nk3) return false;
    if (ch.state.idx_in_S[nk1] != -1) return false;
    if (ch.state.idx_in_S[nk2] != -1) return false;
    if (ch.state.idx_in_S[nk3] != -1) return false;
    if (ch.in_tabu(nk1) || ch.in_tabu(nk2) || ch.in_tabu(nk3)) return false;

    // Step 4: Δ combined + single Metropolis.
    long long delta = ch.state.delta_triple_replace(k1, nk1, k2, nk2, k3, nk3);
    bool accept;
    if (delta <= 0) {
        accept = true;
    } else {
    ...
    int old1 = ch.state.S[k1], old2 = ch.state.S[k2], old3 = ch.state.S[k3];
    ch.state.apply_triple_replace(k1, nk1, k2, nk2, k3, nk3);
    ch.push_tabu(old1);
    ch.push_tabu(old2);
    ch.push_tabu(old3);
    ch.st_P3.accepted++;
    if (delta < 0) ch.st_P3.impr++;
    return true;
}
...
static void mix_recal(Chain& ch)
{
    ...
    double pP3 = prod(ch.st_P3);    // Engine #2 NEW
    ...
    bump(ch.mix.P1_, pP1);
    bump(ch.mix.P2_, pP2);
    bump(ch.mix.P3_, pP3);    // Engine #2 NEW
    bump(ch.mix.Q_,  pQ);
...
int main(int argc, char** argv)
{
    fprintf(stdout,
        "==========================================================\n"
        "  DOSPIRATASMUYPIRATASSONCOJONESNOBRAVATAS — Engine #2\n"
        "  LUNA project, K(13) >= 1155 record attempt\n"
        "  Q(zeta_12) cyclotomic + Move P3 + N_TARGET sweep\n"
        "  Mac M2 25%% CPU, 5-chain PT, hot ladder\n"
        "==========================================================\n");
    std::fflush(stdout);

    if (argc < 2) {
        fprintf(stderr,
            "Usage: %s <ze99_pool_file> [options]\n"
            "  --mode=v3-sanity        : reproduce v3 plateau gbest=2 (sanity test)\n"
            "  --mode=pattern-I        : engine #2 Pattern I + sweep (RECORD ATTACK)\n"
            "  --pool-i=<file>         : Pattern I pool file (Encerador's Phase 3)\n"
            "  --sweep=N1,N2,...       : N_TARGET sweep values (default %d,%d,%d,%d,%d,%d,%d)\n"
            "  --seed-mode=ze99|random|alternating  (default alternating)\n",
            argv[0],
            DEFAULT_SWEEP[0], DEFAULT_SWEEP[1], DEFAULT_SWEEP[2],
            DEFAULT_SWEEP[3], DEFAULT_SWEEP[4], DEFAULT_SWEEP[5], DEFAULT_SWEEP[6]);
        return 1;
    }
    const char* pool_path = argv[1];

    // Engine #2 NEW: mode + pool-i + sweep flags
    enum EngineMode { MODE_V3_SANITY, MODE_PATTERN_I };
    EngineMode engine_mode = MODE_V3_SANITY;  // safe default
    const char* pool_i_path = nullptr;
    std::vector<int> sweep_targets;

    enum SeedMode { SEED_ZE99, SEED_RANDOM, SEED_ALT } seed_mode = SEED_ALT;
    for (int a = 2; a < argc; a++) {
        std::string arg = argv[a];
        if (arg == "--mode=v3-sanity")           engine_mode = MODE_V3_SANITY;
        else if (arg == "--mode=pattern-I")      engine_mode = MODE_PATTERN_I;
        else if (arg.rfind("--pool-i=", 0) == 0) pool_i_path = argv[a] + 9;
        else if (arg.rfind("--sweep=", 0) == 0) {
            std::string v = arg.substr(8);
            std::stringstream ss(v);
            std::string tok;
            while (std::getline(ss, tok, ',')) {
                sweep_targets.push_back(std::stoi(tok));
            }
        }
        else if (arg == "--seed-mode=ze99")        seed_mode = SEED_ZE99;
    ...
    // Sweep defaults per mode
    if (sweep_targets.empty()) {
        if (engine_mode == MODE_V3_SANITY) {
            sweep_targets = {1155};  // sanity = single target
        } else {
            sweep_targets = DEFAULT_SWEEP;
        }
    }

    // Validate sweep targets
    for (int N : sweep_targets) {
        if (N < 1100 || N > N_TARGET_MAX) {
            fprintf(stderr, "[FATAL] sweep target %d out of valid range [1100, %d]\n",
                    N, N_TARGET_MAX);
            return 1;
        }
    }

    // Pattern I requires pool-i
    if (engine_mode == MODE_PATTERN_I && pool_i_path == nullptr) {
        fprintf(stderr, "[FATAL] --mode=pattern-I requires --pool-i=<file>\n");
        return 1;
    }
    ...
    fprintf(stdout, "[CONFIG] engine_mode=%s\n",
            engine_mode == MODE_V3_SANITY ? "v3-sanity" : "pattern-I");
    fprintf(stdout, "[CONFIG] sweep targets:");
    for (int N : sweep_targets) fprintf(stdout, " %d", N);
    fprintf(stdout, "\n[CONFIG] per-target budget: %lld iters base + %d extensions × %lld = %lld cap\n",
            ITERS_PER_CHAIN_BASE, MAX_EXTENSIONS, EXTEND_BY,
            ITERS_PER_CHAIN_BASE + MAX_EXTENSIONS * EXTEND_BY);
    fprintf(stdout, "[CONFIG] plateau auto-stop: %lld iters\n", PLATEAU_AUTOSTOP);
    ...
    // 2. Extend with Pattern B + Pattern F (engine #2: A and E dropped)
    // Pattern A: F-MASTER subset, dropped per V7 §5.3.
    // Pattern E: F-cinco-1 closed, dropped per V7 §5.3.
    // Pattern B: 2000 (Z[√3]-mixed complement)
    // Pattern F: 4000 (largest share, axial-filtered)
    // Total ~6000 + ZE99 1154 = 7154 baseline. Pattern I added below.
    int N_EXTRA = 6000;
    std::vector<Vec> pool = extend_pool_cross_radical(pool_base, rng, N_EXTRA);
    fprintf(stdout, "[POOL] extended pool size after F40-filtered F+B: %zu (base %zu + extra)\n",
            pool.size(), pool_base.size());

    // 2c. Engine #2 NEW: load Pattern I = Q(ζ_12) cyclotomic if requested
    if (engine_mode == MODE_PATTERN_I) {
        fprintf(stdout, "[POOL_I] loading Pattern I from %s ...\n", pool_i_path);
        std::vector<Vec> pattern_I = load_pool_pattern_I(pool_i_path, pool_base);
        size_t before = pool.size();
        // Append Pattern I to pool, deduping against existing pool
        std::unordered_set<size_t> existing_hashes;
        VecHash hasher;
        for (const auto& v : pool) existing_hashes.insert(hasher(v));
        int n_added = 0, n_pool_dup = 0;
        for (const auto& v : pattern_I) {
            size_t h = hasher(v);
            if (existing_hashes.count(h)) {
                bool dup = false;
                for (const auto& w : pool) { if (v == w) { dup = true; break; } }
                if (dup) { n_pool_dup++; continue; }
            }
            pool.push_back(v);
            existing_hashes.insert(h);
            n_added++;
        }
        fprintf(stdout, "[POOL_I] added %d new candidates; %d pool-dups skipped. "
                "Pool size now: %zu (was %zu)\n",
                n_added, n_pool_dup, pool.size(), before);
    } else {
        fprintf(stdout, "[POOL_I] mode=v3-sanity, Pattern I NOT loaded (skipped)\n");
    }
    ...
    // ===== ENGINE #2 SWEEP LOOP =====
    // Iterate over sweep_targets; per target, run full SA with auto-stop.
    int best_target_N = -1;
    long long best_target_viol = LLONG_MAX;
    bool any_record = false;

    auto sweep_t_start = std::chrono::steady_clock::now();

    for (int sw_idx = 0; sw_idx < (int)sweep_targets.size(); sw_idx++) {
        int N_this = sweep_targets[sw_idx];
        g_N_target = N_this;  // RUNTIME N_TARGET (engine #2 NEW)

        fprintf(stdout, "\n");
        fprintf(stdout, "##########################################################\n");
        fprintf(stdout, "## SWEEP TARGET %d/%zu : N_TARGET = %d\n",
                sw_idx + 1, sweep_targets.size(), N_this);
        fprintf(stdout, "## (justified by 'natural-family closure size N*(A) "
                "family-dependent', per Encerador audit addendum 06 May 2026)\n");
        fprintf(stdout, "##########################################################\n");
        std::fflush(stdout);
    ...
    for (int c = 0; c < N_CHAINS; c++) {
        ...
        chains[c].mix = MoveMix{30, 20, 15, 20, 10, 15, 10};  // fallback default
        // Order: S, P1, P2, P3 (NEW), Q, K, R
    }
    ...
    for (int c = 0; c < N_CHAINS; c++) {
        ...
        auto probe_one = [&](int move_idx, ProbeStats& st) {
            chains[c].mix = MoveMix{0,0,0,0,0,0,0};
            switch (move_idx) {
              ...
              case 4: chains[c].mix.P3_ = 100; break;  // Engine #2 NEW
            }
        ...
        probe_one(2, ps_P2);
        probe_one(3, ps_Q);
        probe_one(4, ps_P3);  // Engine #2 NEW
        ...
        chains[c].st_P3 = MoveStats{};  // P3 stats stay clean for run
        ...
        chains[c].mix.P3_ = pick_w(pP3);   // Engine #2 NEW
        chains[c].mix.Q_  = pick_w(pQ);
        chains[c].mix.K_  = 10;
        chains[c].mix.R_  = 10;

        fprintf(stdout, "[COST_PROBE chain=%d] "
                "S=%.3f%% P1=%.3f%% P2=%.3f%% P3=%.3f%% Q=%.3f%% -> "
                "mix S=%d P1=%d P2=%d P3=%d Q=%d K=%d R=%d\n",
                c, pS*100, pP1*100, pP2*100, pP3*100, pQ*100,
                chains[c].mix.S_, chains[c].mix.P1_,
                chains[c].mix.P2_, chains[c].mix.P3_,
                chains[c].mix.Q_, chains[c].mix.K_, chains[c].mix.R_);
    }
    std::fflush(stdout);
    ...
    long long iter = 0;
    long long iters_budget = ITERS_PER_CHAIN_BASE;
    int extensions = 0;
    ...
    while (iter < iters_budget) {
        ...
        for (int c = 0; c < N_CHAINS; c++) {
            ...
            if (pv < chains[c].best_viol) {
                ...
                // Engine #2 NEW: D36 axial-recycling check at every gbest
                // descent (paper v14 §META v10 + V7 §3.5).
                // If config contains F40 axial, this descent is suspect:
                // axial-recycled finds are NOT novel per paper v14 D36.
                if (pv <= 5) {  // Only check at low cset to avoid spam
                    int n_axials = 0;
                    int axial_idx_first = -1;
                    for (int k = 0; k < N_TARGET; k++) {
                        if (is_F40_axial(pool[chains[c].state.S[k]])) {
                            n_axials++;
                            if (axial_idx_first < 0) axial_idx_first = k;
                        }
                    }
                    if (n_axials > 0) {
                        fprintf(stdout, "[D36-ALERT chain=%d iter=%lld pv=%lld] "
                                "config contains %d F40 axial(s) "
                                "(first at S[%d]); descent SUSPECT per "
                                "paper v14 D36 axial-recycling.\n",
                                c, iter, pv, n_axials, axial_idx_first);
                        std::fflush(stdout);
                    }
                }
                ...
                if (pv == 0) {
                    ...
                    if (fresh == 0) {
                        // Engine #2 NEW: also gate on D36 axial absence for
                        // record claim. Paper v14 D36: F40-axial-config is
                        // NOT a novel record.
                        int n_axials = 0;
                        for (int k = 0; k < N_TARGET; k++) {
                            if (is_F40_axial(pool[chains[c].state.S[k]])) {
                        ...
                        if (n_axials == 0) {
                            ...
                            fprintf(stderr, "[D36-RECORD-REJECTED chain=%d iter=%lld] "
                                    "pv=0 but config has %d F40 axials. "
                                    "NOT a novel record per paper v14 D36. "
                                    "Continuing search.\n",
                                    c, iter, n_axials);
                        }
                    } else {
        ...
        if (iter > 0 && iter % 1'000'000 == 0) {
            ...
            for (int c = 0; c < N_CHAINS; c++) {
                ...
                fprintf(stdout, "  chain %d T=%.3f viol=%lld best=%lld | "
                        ...
                        "P3:%.2f%%/%.3f%% Q:%.2f%%/%.3f%% K:%.2f%%/%.3f%% "
                        "R:%.2f%%/%.3f%% | mix=%d/%d/%d/%d/%d/%d/%d\n",
                        c, ch.T_base,
                        ...
                        pct(ch.st_P3.accepted, ch.st_P3.tries),  // engine #2
                        pct(ch.st_P3.impr, ch.st_P3.tries),      // engine #2
                        ...
                        ch.mix.S_, ch.mix.P1_, ch.mix.P2_, ch.mix.P3_,
        ...
        // Engine #2 NEW: plateau auto-stop per N_TARGET (Architect's
        // instruction 06 May 2026). If gbest_viol has not improved in
        // PLATEAU_AUTOSTOP iters across ALL chains, stop early.
        if (iter > PLATEAU_AUTOSTOP) {
            long long last_global_impr = 0;
            for (int c = 0; c < N_CHAINS; c++) {
                if (chains[c].last_impr_iter > last_global_impr) {
                    last_global_impr = chains[c].last_impr_iter;
                }
            }
            if (iter - last_global_impr >= PLATEAU_AUTOSTOP) {
                fprintf(stdout, "[PLATEAU-AUTOSTOP TARGET=%d] no improvement "
                        "in last %lld iters (last impr at iter=%lld); "
                        "stopping target early at iter=%lld\n",
                        N_TARGET, PLATEAU_AUTOSTOP, last_global_impr, iter);
                break;
            }
        }
    ...
    // 9. Dump for D190 paso 4 (independent Python verify) - engine #2
    char dump_path[512];
    snprintf(dump_path, sizeof(dump_path),
             "DOSPIRATAS_E2_dump_target%d_viol%lld.txt",
             N_TARGET, gbest_viol);
    dump_config(polish_state, pool, dump_path, gbest_viol);
    fprintf(stdout, "[DUMP TARGET=%d] written to %s\n", N_TARGET, dump_path);

    // 10. Final verdict
    fprintf(stdout, "==========================================================\n");
    if (gbest_viol == 0) {
        fprintf(stdout, "*** RECORD CANDIDATE for N=%d with pair_violations=0 ***\n"
                        "*** D190 paso 2/3/4 MANDATORY before any claim ***\n",
                N_TARGET);
        any_record = true;
    } else {
        fprintf(stdout, "[CIERRE TARGET=%d] best pair_violations=%lld\n"
                        "         (record requires pair_violations=0)\n",
                N_TARGET, gbest_viol);
    }
    fprintf(stdout, "==========================================================\n");

    // Per-chain move-stats summary (engine #2: includes P3)
    for (int c = 0; c < N_CHAINS; c++) {
        const Chain& ch = chains[c];
        fprintf(stdout, "[CHAIN %d T=%.3f] best=%lld | reheats=%d\n"
                ...
                ch.st_P3.tries, ch.st_P3.accepted, ch.st_P3.acc_rate()*100, ch.st_P3.impr,
    ...
    fprintf(stdout, "[TARGET=%d WALL-CLOCK] %.1f s = %.2f h\n",
            N_TARGET, total_secs, total_secs / 3600.0);
    fprintf(stdout, "[TARGET=%d ITERS] %lld iterations | %.0f iters/sec\n",
            N_TARGET, iter, iter / std::max(total_secs, 1.0));

    // Update best across sweep
    if (gbest_viol < best_target_viol) {
        best_target_viol = gbest_viol;
        best_target_N = N_TARGET;
    }

    // Early stop: if record found at this target, no need to continue sweep
    if (gbest_viol == 0) {
        fprintf(stdout, "[SWEEP] record found at N=%d, stopping sweep\n", N_TARGET);
        break;
    }

    }  // end for sw_idx (sweep loop)

    // ===== SWEEP SUMMARY =====
    auto sweep_t_end = std::chrono::steady_clock::now();
    double sweep_total_secs = std::chrono::duration<double>(sweep_t_end - sweep_t_start).count();
    fprintf(stdout, "\n");
    fprintf(stdout, "##########################################################\n");
    fprintf(stdout, "## SWEEP COMPLETE\n");
    fprintf(stdout, "## Total wall-clock: %.1f s = %.2f h\n",
            sweep_total_secs, sweep_total_secs / 3600.0);
    fprintf(stdout, "## Best target: N=%d with pair_violations=%lld\n",
            best_target_N, best_target_viol);
    if (any_record) {
        fprintf(stdout, "## *** RECORD CANDIDATE: K(13) >= %d ***\n", best_target_N);
        fprintf(stdout, "## D190 paso 2/3/4 MANDATORY before claim\n");
    } else {
        fprintf(stdout, "## No record found across sweep. Wall #%d candidate\n"
                        "## (engine #2 closure, F-dospiratas-2 register if HUB topology)\n",
                11);
    }
    fprintf(stdout, "##########################################################\n");
    return 0;
